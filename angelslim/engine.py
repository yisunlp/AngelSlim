...
class Engine:
    ...
    def prepare_model(
        ...
        # Normalize device_map for DeepSpeed ZeRO / distributed training:
        # YAML configs often use ``None`` / the string ``"None"`` / ``"distributed"``
        # to mean "no pre-placement, let DeepSpeed shard", but HF only accepts
        # Python ``None`` there.
        if isinstance(device_map, str) and device_map.lower() in (
            "none",
            "distributed",
        ):
            device_map = None
